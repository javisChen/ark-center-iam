...
CREATE TABLE IF NOT EXISTS `kt`.`iam_user` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uk_phone` (`phone` ASC, `is_deleted` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `kt`.`iam_permission_role_rel` (
  ...
  `modifier` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新人',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uk_permission_id_role_id` (`permission_id` ASC, `role_id` ASC) VISIBLE)
ENGINE = InnoDB
COMMENT = '角色与权限关联表';
...
CREATE TABLE IF NOT EXISTS `kt`.`iam_api_category` (
  ...
  UNIQUE INDEX `uk_name_application_id` (`application_id` ASC, `name` ASC, `is_deleted` ASC) VISIBLE)
COMMENT = 'Api分类表';
...
CREATE TABLE IF NOT EXISTS `kt`.`iam_api` (
  ...
  INDEX `idx_is_deleted` (`is_deleted` ASC) VISIBLE,
  INDEX `idx_application_id` (`application_id` ASC, `is_deleted` ASC) VISIBLE,
  INDEX `fk_upms_api_category_idx` (`category_id` ASC) VISIBLE)
COMMENT = 'api表';
